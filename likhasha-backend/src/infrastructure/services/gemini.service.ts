...
function getSystemPrompt(type: string, language: string, accent?: string, mood?: string): string {
...
const LOCAL_FALLBACKS: Record<string, Record<string, Record<string, string[]>>> = {
  quote: {
    en: {
      romantic: [
...
export class GeminiService {
  private static getLocalFallback(input: string, type: string, language: string): string {
    ...
    logger.info(`✨ Premium Local Creative Fallback Engine [Type: ${normType}, Lang: ${normLang}, Mood: ${detectedMood}]`);
    ...
    if (!langPool) {
      ...
      if (normLang === 'mr' || normLang === 'as') langPool = categoryPool['hi'];
      else if (normLang === 'ta' || normLang === 'te' || normLang === 'bn' || normLang === 'gu' || normLang === 'kn' || normLang === 'ml' || normLang === 'pa' || normLang === 'or') langPool = categoryPool[normLang] || categoryPool['roman'];
      else if (['es', 'fr', 'pt', 'de', 'it'].includes(normLang)) langPool = categoryPool['en'];
      else if (['ar'].includes(normLang)) langPool = categoryPool['ur'];
      else langPool = categoryPool['roman'] || categoryPool['en'];
    }
  ...
  static async generateContent(input: string, type: string, language: string, accent?: string, mood?: string): Promise<string> {
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        if (generatedText && generatedText.trim().length > 0) {
          logger.info(`✅ Generated content using ${modelName}`);
          return generatedText.trim();
        }
      } catch (error: any) {
        logger.warn(`⚠️ Model ${modelName} failed: ${error?.message || error}`);
      }
    }
    ...
    try {
      ...
      logger.error('CRITICAL: Local fallback engine failed!', fallbackError);
      throw new AppError('Creative writing service temporarily unavailable.', 502, 'AI_SERVICE_ERROR');
    }
  }
}
